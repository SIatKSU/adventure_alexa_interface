...
public class ACSpeechlet implements SpeechletV2 {
	...
	private SpeechletResponse getActionResponse(Session session,
		...
		if (currNode.eventType == MONSTER_EVENT) {
			...
			if (userActionString.equals("attack")) {
				...
				if (playerAttackDmg == 0) {
					...
					gameSpeech.append("You hit the ").append(currNode.monsterName).append(" with your ")
							.append(currAdventure.weaponName).append(", for ").append(Integer.toString(playerAttackDmg))
							.append(" damage.  ");
				}
				if (monsterHealth <= 0) {
					// monster is dead!
					gameSpeech.append("The ").append(currNode.monsterName).append(" is dead!");
					...
					if ((currNode.getNextActions() == null) || (currNode.getNextActions().size() == 0)) { // last node;
						...
						if ((nextEvent.eventType != MONSTER_EVENT)
					...
					int monsterAttackDmg = rand.nextInt(currNode.maxDamage - currNode.minDamage + 1)
							+ currNode.minDamage;
					gameSpeech.append("<break time=\"0.5s\"/>");

					if (monsterAttackDmg == 0) {
						gameSpeech.append("The ").append(currNode.monsterName).append(" misses!  ");
					} else {
						gameSpeech.append("The ").append(currNode.monsterName).append(" hits you with its ")
								.append(currNode.weaponName).append(", for ").append(Integer.toString(monsterAttackDmg))
								.append(" damage.  ");
					}
	...
	private StringBuilder buildActionList(ZEvent node) {
		...
		if (node.eventType == MONSTER_EVENT) {
